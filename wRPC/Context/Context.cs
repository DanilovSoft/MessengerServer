                        ...
                        using (var frameMem = new MemoryPoolStream(WebSocketMaxFrameSize))
                        {
                            ...
                            do
                            {
                                ...
                                try
                                {
                                    ...
                                    webSocketMessage = await socketQueue.WebSocket.ReceiveAsync(segment, CancellationToken.None).ConfigureAwait(false);
                                }
                                catch (Exception ex)
                                // Обрыв соединения.
                                {
                                    ...
                                    // Не удалось десериализовать заголовок.
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            using (var cts = new CancellationTokenSource(3000))
                                                await socketQueue.WebSocket.CloseAsync(WebSocketCloseStatus.ProtocolError, "Ошибка десериализации заголовка.", cts.Token).ConfigureAwait(false);
                                        }
                                        catch (Exception ex)
